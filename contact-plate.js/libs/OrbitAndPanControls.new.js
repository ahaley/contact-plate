...
THREE.OrbitAndPanControls = function ( object, domElement ) {
    ...
    var rotateTargetStart = new THREE.Vector2();
    var rotateTargetEnd = new THREE.Vector2();
    var rotateTargetDelta = new THREE.Vector2();
	...
	var STATE = { NONE : -1, ROTATE : 0, DOLLY : 1, PAN : 2, TOUCH_ROTATE : 3, TOUCH_DOLLY : 4, TOUCH_PAN : 5, ROTATE_TARGET : 6 };
    ...
    this.rotateTargetLeft = function (angle) {
    ...
    this.rotateTargetUp = function (angle) {
	...
	function onMouseDown( event ) {
        ...
        if (event.button === 0 && event.shiftKey) {
            ...
            state = STATE.ROTATE_TARGET;

            rotateTargetStart.set(event.clientX, event.clientY);
	...
	function onMouseMove( event ) {
		...
		} else if (state === STATE.ROTATE_TARGET) {
            if ( scope.noRotate === true ) { return; }

            rotateTargetEnd.set(event.clientX, event.clientY);
            rotateTargetDelta.subVectors(rotateTargetEnd, rotateTargetStart);

            var angle = 2 * Math.PI * rotateTargetDelta.x
                / scope.domElement.width * scope.rotateSpeed;

            scope.rotateTargetLeft(angle);

            var upAngle = 2 * Math.PI * rotateTargetDelta.y
                / scope.domElement.height * scope.rotateSpeed

            scope.rotateTargetUp(upAngle);

            rotateTargetStart.copy(rotateTargetEnd);
        }
